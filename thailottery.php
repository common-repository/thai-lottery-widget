...
//Only search box
class wp_thailottery_plugin4 extends WP_Widget {
	// constructor
	function __construct() {
			parent::__construct(
			'wp_thailottery_plugin4', // Base ID
			'1 แสดงช่องตรวจหวย Thai Lottery Widget style 4', // Name
			array( 'description' => 'Widget แสดงเฉพาะช่องตรวจหวย (Only search box).' ) // Args
		);
}
function form($instance) {
if( $instance) {
...
<p>
การใช้งาน Shortcode สำหรับแสดงช่องค้นหา ตรวจหวย<br/>
<strong>[thailottery4] หรือปรับแต่งขนาด [thailottery4 width="100%" height="410"]</strong><br/>
นำโค้ดด้านบนนี้ไปวางในส่วนของหน้าเพจหรือหน้าเนื้อหาเพื่อแสดงผล
<br/><em>width=ความกว้าง ค่าปกติ 100% height=ความสูง ค่าปกติ 410</em>
</p>
<?php
}
// update widget
...
$myheight='410';
   ...
   echo '<iframe src=https://www.lottery.co.th/numbers width='.$mywidth.' height='.$myheight.' frameborder=0></iframe>';
   echo $after_widget;
}
}
...
function sthailottery4($atts, $content = null ) {
    $a = shortcode_atts( array(
        'width' => '100%',
        'height' => '410',
    ), $atts );
       return '<iframe src=https://www.lottery.co.th/numbers width='.$a['width'].' height='.$a['height'].' frameborder=0></iframe>';
}
...
add_shortcode('thailottery4', 'sthailottery4');
...
add_action('widgets_init', create_function('', 'return register_widget("wp_thailottery_plugin4");'));
